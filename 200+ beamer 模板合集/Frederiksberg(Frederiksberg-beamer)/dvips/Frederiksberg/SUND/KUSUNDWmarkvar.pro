...
% Defines SUND watermark code, variant for dark background.
...
{
  /KUSUNDwmarkvar_colordef {
    _kuwmark_colordef_first
    /id 22 string def
    id 0 ({KUSUNDwmarkvar) putinterval
    id 15 tmpstr putinterval
    id 21 (}) putinterval
    [ /_objdef id cvx exec /BBox [96 508 178 608] /BP pdfmark
    save r g b setrgbcolor _kusund_wmark_var restore
    _kuwmark_colordef_last
  } bind def
  /KUSUND_wmark_var /KU_wmark_var load def
}?pdfmark
{
  /KUSUNDwmarkvar_colordef /KUwmark_colordef load def
  /KUSUND_wmark_var {_kuwmark_color aload pop setrgbcolor _kusund_wmark_var} bind def
}?notpdfmark
